...
@section('container')
    ...
    <div class="card mb-3">
        @if ($posts[0]->image)
          <div style="max-height: 350px; overflow:hidden">
              <img src="{{ asset('storage/'.$posts[0]->image) }}" alt="{{ $posts[0]->category->name }}" class="img-fluid mx-auto">
          </div>
        @else
            <img src="https://source.unsplash.com/random/640x354?{{ $posts[0]->category->name }}" class="card-img-top" alt="{{ $posts[0]->category->name }}">
        @endif
        ...
        <div class="row row-cols-1 row-cols-md-4 g-4 mb-3">
            ...
            <div class="col">
                <div class="card h-100">
                    <p class="position-absolute px-4 py-2" style="background-color: rgba(0, 0, 0, 0.6)"><a href="/blog?category={{ $post->category->slug }}" class="text-decoration-none text-white">{{ $post->category->name }}</a></p>
                    @if ($post->image)
                        <img src="{{ asset('storage/'.$post->image) }}" alt="{{ $post->category->name }}" class="img-fluid mx-auto">
                    @else
                        <img src="https://source.unsplash.com/random/640x354?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}">
                    @endif
                    <div class="card-body">
                        ...
                        <p class="card-text mb-9">{{ $post->excerpt }}</p>
    ...
    @else
        <p class="fs-4 text-center alert alert-info" role="alert">No Post found!</p>
    @endif
